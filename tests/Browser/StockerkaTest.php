...
class StockerkaTest extends DuskTestCase
{

    private $config = [
        'url' => 'https://www.stockerka.sk/byvanie',
        'wait_time' => 3000,
        'name' => 'Stockerka',
        'slug' => 'stockerka',
        'pricelist_head_element' => 'div#table-container table.pricelist thead',
        'pricelist_body_element' => 'table.pricelist tbody',
    ...
    public function scrapeData()
    {
        ...
        $this->browse(function (Browser $browser) {
            ...
            // $browser->driver->executeScript("document.querySelector('button.cookie-bar__button.cookie-bar__accept_all.cookie-bar__button__full').click();");
            // $browser->driver->executeScript('document.querySelectorAll(\'.element-class\').forEach(element => element.style.display=\'initial\');');
            // $browser->pause($this->config['wait_time']);
            $browser->driver->executeScript("document.querySelector('html').style.overflow = 'initial';");
            $browser->driver->executeScript("document.querySelector('body').style.overflow = 'initial';");
            ...
            foreach($trs as $trKey => $tr) {
                ...
                if($this->config['pricelist_excluded_rows']) {
                    ...
                    if(in_array($this->config['pricelist_allowed_rows_class'], $classes)) {
                        $tds = $browser->elements($this->config['pricelist_body_element'].' a.tr:nth-child('.($rowNum).') td');
                    ...
                    //$tds = $browser->elements($this->config['pricelist_body_element'].' a tdtr:nth-child('.($rowNum).') td');
                    $tds = $browser->elements($this->config['pricelist_body_element'].' > tr:nth-child('.($rowNum).') > td');
                    $cleanedTds = [];

                    dd(count($tds), $header['column_count']);
                    // NOTE: remove hidden elements
                    if(count($tds) > $header['column_count']) {
                        foreach($tds as $key => $td) {
                            $classes = explode(' ', $td->getAttribute('class'));
