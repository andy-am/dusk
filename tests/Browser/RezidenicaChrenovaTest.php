...
class RezidenicaChrenovaTest extends DuskTestCase
{

    private $config = [
        'url' => 'https://rezidenciachrenova.sk/byty/',
        'wait_time' => 2000,
        'name' => 'Rezidencia Chrenova',
        'slug' => 'rezidenciachrenova',
...
//            $browser->driver->executeScript('document.querySelectorAll(\'.element-class\').forEach(element => element.style.display=\'initial\');');
            ...
            //dump($header['column_count'], $header['column_names'], count($trs));
            ...
            foreach($trs as $trKey => $tr) {
                ...
                if($this->config['pricelist_excluded_rows']) {
                    ...
                    $tds = $browser->elements($this->config['pricelist_body_element'].' > tr:nth-child('.($trKey+1).') td:not(:last-child)');
                    $cleanedTds = [];
                    
                    //dump(count($tds),$header['column_count']);
